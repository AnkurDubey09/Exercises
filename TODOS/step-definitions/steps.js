...
Then("verify todos item {string} is edited with {string}", async(todoItem, editedTodo) => {
    const editedItem = await getElement(`//label[contains(text(),'${todoItem}')]`);
    expect(await editedItem.getText()).toBe(`${editedTodo}`);
});

Then("verify the {string} is added in todos list", async(expectedAddedItem) => {
    const actualItemInList = await getElements(TodosPage.allTodos);
...
Then("verify {string} in footer", async(footerCountText) => {
    const footerCount = await getElement(TodosPage.footerTodosCount);
    expect(await footerCount.getText()).toBe(footerCountText);
});

Then("verify the todos page is loaded", async()=>{
    const todosHeader = await getElement(TodosPage.todosHeader);
    const todosInput = await getElement(TodosPage.todosInput);
    const footers = await getElements(TodosPage.todosFooters);
...
Then('verify footer links are not shown', async () => {
    const allTodoslink = await getElement(TodosPage.allTodosLink);
    const activeTodoslink = await getElement(TodosPage.activeTodosLink);
    const completedTodoslink = await getElement(TodosPage.completedTodosLink);
...
Then("verify {string} todos under {string} tab", async(count, tab) => {
    let todosActualCount = (await getElements(TodosPage.allTodos)).length;
...
Then("verify the order of the todos", async() => {
    const actualTododsOnPage = await getElements(TodosPage.allTodos);
